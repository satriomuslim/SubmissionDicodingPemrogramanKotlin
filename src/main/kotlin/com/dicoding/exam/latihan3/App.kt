...
fun <T> checkType(args: T): String {
    if(args is Int) return "Yes! it's Integer"
    if(args is String) return "Yes! it's String"
    if(args is Boolean) return "Yes! it's Boolean"
    if(args is Double) return "Yes! it's Double"
    if(args is List<*>) return "Yes! it's List"
    if(args is Map<*, *>) return "Yes! it's Map"
    return "it's null"
}
